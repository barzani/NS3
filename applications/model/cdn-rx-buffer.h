...
#ifndef CDN_RX_BUFFER_H
#define CDN_RX_BUFFER_H
 ...
 */
class CdnRxBuffer : public Object
{
public:
  /**
   ...
   */
  uint32_t NextRxSequence (void) const;
  /**
   ...
   */
  uint32_t MaxRxSequence (void) const;
  /**
   ...
   */
  void SetNextRxSequence (const uint32_t& s);
  /**
   * \brief Set the FIN Sequence number (i.e., the one closing the connection)
   * \param s the Sequence number
   */
  void SetFinSequence (const uint32_t& s);
  /**
   ...
   */
  bool Add (Ptr<Packet> p, SeqTsHeader& seqTshdr);
...
public:
  ...
  typedef std::map<uint32_t, Ptr<Packet> >::iterator BufIterator;
  TracedValue<uint32_t> m_nextRxSeq; //!< Seqnum of the first missing byte in data (RCV.NXT)
  uint32_t m_finSeq;                 //!< Seqnum of the FIN packet
  ...
  std::map<uint32_t, Ptr<Packet> > m_data; //!< Corresponding data (may be null)
};
