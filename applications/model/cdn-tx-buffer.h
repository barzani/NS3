...
#ifndef CDN_TX_BUFFER_H
#define CDN_TX_BUFFER_H
 ...
 * \ingroup tcp
 *
 * \brief class for keeping the data sent by the application to CDN, i.e.
 *        the sending buffer.
 */
class CdnTxBuffer : public Object
{
public:
  /**
   ...
   */
  CdnTxBuffer (uint32_t n = 0);
  virtual ~CdnTxBuffer (void);
   ...
   * \returns the first byte's sequence number
   */
  uint32_t HeadSequence (void) const;

  /**
   * Returns the last byte's sequence number + 1
   * \returns the last byte's sequence number + 1
   */
  uint32_t TailSequence (void) const;
   ...
   */
  uint32_t SizeFromSequence (const uint32_t& seq) const;
   ...
   * \returns a packet
   */
  Ptr<Packet> CopyFromSequence (uint32_t numBytes, const uint32_t& seq);
   ...
   */
  void SetHeadSequence (const uint32_t& seq);
...
private:
  ...
  typedef std::list<uint32_t>::iterator NumBufIterator;
